...
data_points = length(ViralData)+length(CD4Data)+length(AlbuminData)+length(GlobulinData);
...
tforward = 0:0.1:170;
 ...
 k = [46083.0686392091,0.161003028125379,1.07778443294132e-08,...
     ...
     5.26530143560413e-10,0.00277981294517048,3];
...
display('Parameters after data fitting Model 3:')
...
dy(1) = r - rho*T*V - d*T;
dy(2) = rho*T*V - delta*T_i - (psi_02*G/(psi + A + G))*T_i*Z;
