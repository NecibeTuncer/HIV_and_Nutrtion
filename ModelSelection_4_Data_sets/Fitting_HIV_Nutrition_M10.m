...
tforward = 0:0.1:170;
 ...
 k = [46083.0686392091,0.161003028125379,1.07778443294132e-08,...
     ...
     5.26530143560413e-10,0.00277981294517048,3, 0.1];


lb = zeros(1,length(k));

[k,fval] =  fminsearchbnd(@err_in_data,k,lb,[],optimset('Display','iter','MaxFunEvals', 1e+4, 'MaxIter', 1e+5));
...
display('Parameters after data fitting Model 6:')
...
dy(1) = r - (rho_0/(1+A_2*G))*T*V - d*T;
dy(2) = (rho_0/(1+A_2*G))*T*V - delta*T_i - (psi_01*A)/(psi + A + G)*T_i*Z;
