...
k = [46083.0686392091,0.161003028125379,1.07778443294132e-08,...
     ...
     5.26530143560413e-10,0.00277981294517048, 0.1, 0.1, 3.30502074055700e-07, 3.30502074055700e-07, 0.1, 0.1];
lb = zeros(1,length(k));
...
dy(1) = r + r_0*G - (rho_0/(1 + A_1*A + A_2*G))*T*V - d*T;
dy(2) = (rho_0/(1 + A_1*A + A_2*G))*T*V - delta*T_i - (psi_02*(psi_01*A + G))/(psi + A + G)*T_i*Z;
dy(3) = p*T_i - c*V - mu_v*G*V;
dy(4) = lambda_z + lambda_z_0*G + b*T_i*Z - mu_z*Z;
