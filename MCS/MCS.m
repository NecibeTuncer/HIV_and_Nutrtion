...
initial_cond = [1304000 0 200 10 3.44 2.57179227742938];
 ...
 total_ARE =  zeros(length(noiselevel), length(true_params));
  ...
  save('Workspace_MCS_At_Data_Points')
...
noise = noiselevel(j);
    ...
    Viral_data = log10(y_trp(t_v_measure(:),3));
    CD4_data =  log10(y_trp(t_cd4_measure(:),1));
    Albumin_data = y_trp(t_alb_measure(:),5);
    Globulin_data = y_trp(t_glob_measure(:),6);

  ViralData = (noise*randn(length(t_v_measure),1))+ Viral_data;
